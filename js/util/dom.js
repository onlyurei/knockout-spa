        ...
        handleSwipe: function (element, callback) {
            if (Dom.isTouchDevice) {
                ...
                element.addEventListener('touchend', function (event) {
                    var touchObject = event.changedTouches[0];
                    distanceX = touchObject.pageX - startX; // get horizontal distance traveled by finger while in contact with surface
                    distanceY = touchObject.pageY - startY; // get vertical distance traveled by finger while in contact with surface
                    ...
                    if (elapsedTime <= allowedTime) { // first condition for awipe met
                        if (Math.abs(distanceX) >= distanceThreshold && Math.abs(distanceY) <= restraint) { // 2nd condition for horizontal swipe met
                            swipeDirection = (distanceX < 0) ? 'left' : 'right'; // if distance traveled is negative, it indicates left swipe
                        }
                        else if (Math.abs(distanceY) >= distanceThreshold && Math.abs(distanceX) <= restraint) { // 2nd condition for vertical swipe met
                            swipeDirection = (distanceY < 0) ? 'up' : 'down'; // if distance traveled is negative, it indicates up swipe
                        }
                    }
