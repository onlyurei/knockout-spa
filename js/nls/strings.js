define({
  "root": {
    "error.403": "Forbidden",
    "error.404": "Not Found",
    "error.500": "Internal Server Error",
    "nav.home": "Home",
    "nav.files": "Files",
    "nav.files-dependencies": "Files Dependencies",
    "nav.resources": "Resources",
    "files.dependencies.instruction": "This is the files (modules) dependencies graph. Click on a node to see its immediate dependencies and dependants. Click on empty space to deselect nodes. Outward arrow from a node indicates that node is a dependency, inward arrow to a node indicates that node is a dependant. You can zoom/pan the graph and drag the nodes around.",
    "join.discussion": "Join the Discussion",
    "join.discussion.text": "Come in to the knockout-spa Gitter room and discuss new features, future goals, general problems or questions, or anything else you can think of.",
    "chat": "Chat",
    "connect": "Connect",
    "sub-path.info": "Type dynamic path after /path/sub-path/ in the browser address bar, then open browser console to see params: /path/sub-path/:param1/:param2"
    /* TODO: add app localized strings */
  },
  "en-gb": true
  /* TODO: add other supported locales */
});
